...
class WP_Nav_Menu_Custom_Fields {

	public static  $field_keys = [

		'_menu_item_svg_icon' => [
			'title' => 'Название иконки с раширением SVG. Пример: icon.svg',
			'desc' => 'Файл svg сохранить в images/icons',
		],
	...
	public static function init(): void {
		...
		if( is_admin() ){
			add_action( 'wp_nav_menu_item_custom_fields', [ __CLASS__, 'add_fileds' ], 10, 5 );
	...
	public static function add_fileds( $item_id, $item, $depth, $args, $menu_id ) {
	...
	public static function nav_menu_start_el( $item_output, $post  ){
		...
		if( $svg ){
            $file_path = TEMPLATEPATH.'/images/icons/'.$svg;
			$svg = file_exists($file_path) ? file_get_contents($file_path) : false;
            //$svg = '<div>'. $svg . '</div>';
		}
